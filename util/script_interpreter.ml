...
let rec step
  : type b a .
    (?log: execution_trace ref ->
     ...
     Tez.t ->
     (b, a) descr -> b stack ->
     (a stack * Context.t) tzresult Lwt.t) =
  fun ?log ctxt ~source ~self ~payer ?visitor amount ({ instr ; loc ; _ } as descr) stack ->
    ...
    let consume_gas_terop : type ret arg1 arg2 arg3 rest.
      ...
      rest stack ->
      ((ret * rest) stack * Context.t) tzresult Lwt.t =
    ...
    let consume_gaz_comparison :
      ...
      t -> t ->
      rest stack ->
      ((Script_int.z Script_int.num * rest) stack * Context.t) tzresult Lwt.t =
